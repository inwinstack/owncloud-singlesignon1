...
namespace OCA\SingleSignOn1\Middleware;

use OCA\SingleSignOn1\AuthInfo;
	...
	public function beforeController($controller, $methodName){
        ...
        if ($this->reflector->hasAnnotation('SSOCORS') &&
            ...
            if(!\OC::$server->getSystemConfig()->getValue("sso_one_time_password")) {
                $tokenVaildator = \OCA\SingleSignOn1\RequestManager::send(\OCA\SingleSignOn1\ISingleSignOn1Request::VALIDTOKEN, $authInfo);
                if (!$tokenVaildator) { 
                    throw new SecurityException('Token expired!', Http::STATUS_UNAUTHORIZED);
                }
            }
            $userInfo = \OCA\SingleSignOn1\RequestManager::getRequest(\OCA\SingleSignOn1\ISingleSignOnRequest::INFO);

			$this->session->logout();
			if(!\OCA\SingleSignOn1\Util::login($userInfo,$authInfo)) {
